...
function heroes_update_stats($request) {
    ...
    if (!$confirmed) {
        ...
        // Update the fields for the revision
        $target_post_id = $revision_id;
    } else {
        // Directly update the published post
        $target_post_id = $hero_id;
    }

    // Update fields for stats
    update_field('bio_fields_max_level', 100, $target_post_id);
    update_field('card_slot', 2, $target_post_id);
    update_field('stat_fields_atk', $atk, $target_post_id);
    update_field('stat_fields_hp', $hp, $target_post_id);
    update_field('stat_fields_def', $def, $target_post_id);
    update_field('stat_fields_crit', $crit, $target_post_id);
    update_field('stat_fields_heal', $heal, $target_post_id);
    update_field('stat_fields_damage_reduction', $damage_reduction, $target_post_id);
    update_field('stat_fields_basic_resistance', $basic_resistance, $target_post_id);
    update_field('stat_fields_light_resistance', $light_resistance, $target_post_id);
    update_field('stat_fields_dark_resistance', $dark_resistance, $target_post_id);
    update_field('stat_fields_fire_resistance', $fire_resistance, $target_post_id);
    update_field('stat_fields_earth_resistance', $earth_resistance, $target_post_id);
    update_field('stat_fields_water_resistance', $water_resistance, $target_post_id);
    update_field('bio_fields_compatible_equipment', $compatible_equipment, $target_post_id);
    update_field('ability_fields_passive_buffs', $passives, $target_post_id);
