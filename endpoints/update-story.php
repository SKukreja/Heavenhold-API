...
add_action('rest_api_init', function () {
    register_rest_route('heavenhold/v1', '/update-story', array(
        'methods'             => 'POST',
        'callback'            => 'heroes_update_story',
        'permission_callback' => '__return_true', // Adjust this according to your permission requirements
    ));
});

function heroes_update_story($request) {
    global $wpdb;
    ...
    // Validate inputs
    if (!isset($hero_id) || !isset($story)) {
        return new WP_Error('invalid_data', 'Hero ID and story are required', array('status' => 400));
    }
    ...
    update_field('bio_fields_story', $story, $target_post_id);

    return array('success' => true, 'post_id' => $target_post_id, 'story' => $story, 'version' => '1.0.0');
}
