...
function heroes_update_portrait($request) {
    ...
    $revision_id = $hero_id;  // Default to updating the published post unless creating a revision
    ...
    $attachment_id = wp_insert_attachment(array(
        ...
        'post_content'   => '',
        'post_status'    => 'inherit'
    ), $uploaded_file['file'], $confirmed ? $hero_id : $revision_id); // Attach to the correct post
    ...
    // Determine which post to update: revision or the published post
    $post_to_update = $confirmed ? $hero_id : $revision_id;

    // Retrieve the current rows in the repeater field
    $existing_portraits = get_field('portrait', $post_to_update);
    ...
    update_field('portrait', $existing_portraits, $post_to_update);
